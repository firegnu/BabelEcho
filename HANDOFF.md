...
- 新增 `babelecho speaker-profiles apply-voice-roles --workspace ... --run-id ... --voice-role-map ... [--overwrite]`，显式 opt-in 把 private voice-role map 应用到单次 run 的 `script/speaker-voices.json`。它只写目标 `run_id` 的 speaker，默认不覆盖已有 `speaker-voices.json`，默认 pipeline 不会自动调用。

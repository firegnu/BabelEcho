...
- 新增 `babelecho speaker-profiles apply-voice-roles --workspace ... --run-id ... --voice-role-map ... [--overwrite]`，显式 opt-in 把 private voice-role map 应用到单次 run 的 `script/speaker-voices.json`。它只写目标 `run_id` 的 speaker，默认不覆盖已有 `speaker-voices.json`，默认 pipeline 不会自动调用。5090D smoke 已应用到 `audio-voice-profile-real-practicalai-zero-trust-8min-20260620`：写出 `speaker_1 -> female_a`，不含 `embedding_artifact` 或 `voice-profiles`，`load_speaker_voice_roles()` 返回同一映射，重复执行默认 `reused`。

...
- 新增 `babelecho speaker-profiles review --alias-map ... --output-json ... [--existing-review ...]`，从私有 alias candidates 生成私有人工审核 contract。默认状态为 `candidate`，可编辑为 `confirmed/rejected/split/ignored`；`--existing-review` 会按 `alias_id` 保留已有人工决定。review 文件只含安全摘要和人工审核字段，不读取/输出 embedding，不发布，也不会被 TTS 路由消费。5090D real smoke 已用五集 Practical AI alias map 生成 `workspace/runs/speaker-alias-review-practicalai-real-five-episodes-20260620.json`：2 个 alias、`candidate=2`，且不含 `embedding_artifact` 或 `voice-profiles`。
- 新增 `babelecho speaker-profiles voice-roles --review ... --output-json ... [--existing-map ...]`，从私有 review contract 生成 confirmed alias 到固定中文 voice role 的私有映射。只有 `review_status=confirmed` 会被映射到 `female_a/male_a/female_b/male_b`；`candidate/rejected/split/ignored` 只进入 `skipped_aliases`。`--existing-map` 会保留已有 `alias_id -> voice_role`，避免跨集重复生成时音色漂移。该 map 仍不被 TTS 消费、不发布、不读取/输出 embedding、不做 voice clone。5090D 已用模拟 confirmed review smoke：`speaker_alias_001 -> female_a`、`speaker_alias_002` 因 `rejected` 跳过；另一次 `--existing-map` smoke 保留 `speaker_alias_001 -> male_b`。
- 新增 `babelecho speaker-profiles apply-voice-roles --workspace ... --run-id ... --voice-role-map ... [--overwrite]`，显式 opt-in 把 private voice-role map 应用到单次 run 的 `script/speaker-voices.json`。它只写目标 `run_id` 的 speaker，默认不覆盖已有 `speaker-voices.json`。新增配置开关 `speaker_voices.mode: apply_voice_role_map` + `speaker_voices.voice_role_map`，可在 `run`、`synthesize` 和 audio-first `audio convert` 进入 TTS 前显式应用 private map；默认 pipeline 不启用。5090D CLI smoke 已应用到 `audio-voice-profile-real-practicalai-zero-trust-8min-20260620`：写出 `speaker_1 -> female_a`，不含 `embedding_artifact` 或 `voice-profiles`，`load_speaker_voice_roles()` 返回同一映射，重复执行默认 `reused`。5090D 配置开关 smoke 已应用到 `audio-voice-profile-real-practicalai-ai-index-8min-20260620`：写出 `speaker_2 -> female_a`，不含私有 embedding 字符串，重复执行默认 `reused`。
...
- `audio_url` 已完成 ingest、normalize、短音频 full-chain、自动边界清理、非 WAV diarization 输入规范化、Hantavirus 类质量门校准、newsletter/podcast promo 片尾清理和 NPR 非 BBC direct-audio 全链路回归。private speaker alias 人工审核 contract 已实现并在 5090D 真实 alias map 上 smoke 通过；confirmed alias 到稳定中文 voice role 的私有映射 contract 已实现并在 5090D smoke 通过；显式 opt-in 应用到单次 run 的 `speaker_voices` 已实现并在 5090D smoke 通过；显式配置开关已实现并在本机与 5090D 覆盖 transcript-first/audio-first synthesize。不要默认启用 alias map，不要进入原主播 voice clone。

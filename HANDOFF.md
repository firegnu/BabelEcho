...
- `audio_url` 已完成 ingest、normalize、短音频 full-chain、自动边界清理、非 WAV diarization 输入规范化、Hantavirus 类质量门校准、newsletter/podcast promo 片尾清理和 NPR 非 BBC direct-audio 全链路回归。private speaker alias 人工审核 contract 已实现并在 5090D 真实 alias map 上 smoke 通过。下一步设计 confirmed alias 到跨集稳定中文 voice role 的私有映射 contract；不要直接把 alias map 接入 TTS。

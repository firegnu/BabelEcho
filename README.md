...
MVP-0 and MVP-0.5 are complete. MVP-1 has selected one deployed TTS model: `CosyVoice-300M-SFT`. Single-speaker scripts use `female_a` by default, explicit `male`/`男` labels use `male_b`, explicit `female`/`女` labels use `female_a`, and 2+ speakers use stable `speaker -> voice_role` mapping with the built-in `female_a / male_b / female_b / male_a` roles. The current focus is real podcast sources and common interview workflows beyond manually supplied transcripts.
...
- Automatic TTS voice-role selection on `CosyVoice-300M-SFT`: unlabeled single speaker uses `female_a`, explicit male/female single speakers use `male_b`/`female_a`, and 2+ speakers use stable `speaker -> voice_role` mapping.

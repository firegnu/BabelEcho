...
- 下一步不要直接安装或接入 SpeechBrain / pyannote embedding / NeMo 等真实重模型。若继续 Route B，应先用 5090D 跑 `tests/test_voice_profile.py` 和 `tests/test_audio_pipeline.py::test_audio_convert_diarize_stage_applies_fixture_voice_profile` fixture smoke；之后再单独写真实 embedding provider 计划。

...
- 下一步计划见 `docs/plans/03-audio-first-asr/03-real-voice-profile-provider.md`：用 5090D `voice_profile.provider=local_cli` 跑真实 SpeechBrain wrapper smoke，确认 `speaker-profiles.json` 合并为 `embedding_status=computed` 且 publish 不暴露 embedding artifact。不要安装到 conda base，不要让 BabelEcho core 直接依赖 SpeechBrain；这仍然不是原主播 voice clone，也不把 embedding 用于 TTS。

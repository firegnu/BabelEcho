...
- MVP-1 固定音色规则已选定并实现：运行默认只用 `CosyVoice-300M-SFT` 的 `tts.voice=sft_builtin_4role`。未启用 speaker voice 推断时，0/1 个 distinct speaker 且没有显式性别标签使用 `female_a`；单个 speaker 标签包含 `male` / `男` 时使用 `male_a`，包含 `female` / `女` 时使用 `female_a`；2 个及以上 distinct speaker 按首次出现顺序映射到 `female_a / male_a / female_b / male_b`。调整原因：用户试听 `male_a / 中文男` 清亮度实验后选择 D 版 EQ，当前 `male_a` 固化为 D 版清亮后处理，男声优先恢复为 `male_a -> male_b`；5090D 预览 `male-a-primary-preview-20260618` 已真实生成 `male_a / male_b / male_a / male_b`，MP3 已拷回本机 ignored `workspace/runs/male-a-primary-preview-20260618/output/audio.mp3`。
- `male_a / 中文男` 清亮度实验已固化 D 版：5090D run `male-a-brightness-experiment-20260618` 产出 A 当前中文男、B 更强 EQ、C 更强 EQ + 半音升调、D 基于 B 再加亮、E 最大亮度试探，用户选择 D；试听文件已拷回本机 ignored `workspace/runs/male-a-brightness-experiment-20260618/variants/`。
- 混合双模型男一实验已生成但未固化默认：5090D run `four-role-hybrid-cosyvoice2-male-a-20260618` 使用 `female_a/female_b/male_b` 继续走 `CosyVoice-300M-SFT`，第二段 `male_a` 单独走昨天的 `CosyVoice2 cross_lingual + cross_lingual_prompt.wav + speed=1.0`，再统一到 `22050 Hz` mono 后 assemble；MP3 已拷回本机 ignored `workspace/runs/four-role-hybrid-cosyvoice2-male-a-20260618/output/audio.mp3`。
- CosyVoice2 男声自然文本语速实验已生成但未固化默认：5090D run `cosyvoice2-male-natural-speed-preview-20260618` 使用全中文播客句子，分别生成 `cross_lingual + speed=1.0` 和 `speed=1.1`，并拷回本机 ignored `workspace/runs/cosyvoice2-male-natural-speed-preview-20260618/output/`；用于判断昨天单固定音色路线是否只是测试文本导致慢和听不清。
- 混合四角色 speed=1.1 男一实验已生成但未固化默认：5090D run `four-role-hybrid-cosyvoice2-male-a-speed11-20260618` 按 `female_a / male_a / female_b / male_b` 顺序生成，只有第二段 `male_a` 使用 `CosyVoice2 cross_lingual + speed=1.1`，其余三段继续 `CosyVoice-300M-SFT`；MP3 已拷回本机 ignored `workspace/runs/four-role-hybrid-cosyvoice2-male-a-speed11-20260618/output/audio.mp3`。
...
1. 先听测 `workspace/runs/on-demand-99pi-karaoke-real-20260618/output/audio.mp3`，确认多 speaker 四角色实际听感。
2. 继续补真实来源的失败诊断和站点/API 边界记录。

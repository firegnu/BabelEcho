...
- MVP-1 固定音色规则已选定并实现：运行默认只用 `CosyVoice-300M-SFT` 的 `tts.voice=sft_builtin_4role`。未启用 speaker voice 推断时，0/1 个 distinct speaker 且没有显式性别标签使用 `female_a`；单个 speaker 标签包含 `male` / `男` 时使用 `male_b`，包含 `female` / `女` 时使用 `female_a`；2 个及以上 distinct speaker 按首次出现顺序映射到 `female_a / male_b / female_b / male_a`。调整原因：真实点播 run 中 `male_a` 作为男一偏低沉，男声优先改为 `male_b -> male_a`；5090D 短预览 `male-role-swap-preview-20260618` 已真实生成 `male_b / male_a / male_b / male_a`，MP3 已拷回本机 ignored `workspace/runs/male-role-swap-preview-20260618/output/audio.mp3`。
- `male_a / 中文男` 清亮度实验已生成但尚未固化默认：5090D run `male-a-brightness-experiment-20260618` 产出 A 当前中文男、B 更强 EQ、C 更强 EQ + 半音升调、D 基于 B 再加亮、E 最大亮度试探，已拷回本机 ignored `workspace/runs/male-a-brightness-experiment-20260618/variants/`；用户已反馈 B 比 A/C 更好，等待用户试听 D/E 后决定是否替换 `male_a` 后处理。
- MVP-1 speaker voice 推断已完成第一版：可在 local config 启用 `speaker_voices.mode: infer_once`，`run`/`synthesize` 会在 TTS 前每集最多调用一次 LLM，根据 speaker 名称和少量上下文推断 `male/female/unknown`，写入 ignored run-local `script/speaker-voices.json`，再由代码稳定映射到 `female_a/male_b/female_b/male_a`。`confidence` 只用于人工复核提示，不阻塞；`unknown` 也会自动获得具体 voice role。若推断失败或文件无效，回退到旧的首次出现规则。

...
```text
你现在在 BabelEcho 项目中工作。请先阅读 resume-prompt.md、HANDOFF.md、docs/plans/README.md 和 docs/plans/01-backend-mvp0/01-local-llm-adapt.md。01.01 DeepSeek LLM Adapt 基线接入已经完成，下一步从本地中文 TTS 接入继续。
...
- 当前已有 DeepSeek API 生成中文口播稿的真实 adapt 基线，但还没有真实 TTS、voice clone、ASR 或真实播客来源接入。
- 下一步只接本地中文 TTS，不要同时推进来源、voice clone、ASR、App 或后台服务。
...
- 01.01 已在 5090D 上完成验收，下一步进入本地中文 TTS 接入。
...
5090D 已完成 DeepSeek API 和 `adapt` 验证。下一步应进入本地中文 TTS 接入，不要同时接真实来源、ASR、voice clone、后台服务或 App。
...
```text
中文口播稿 script/zh.json -> 本地中文 TTS -> segments/*.wav
```
...
- 5090D 上本地 TTS wrapper 可把一句中文生成 wav。
- `babelecho synthesize` 使用 `tts.provider: local_cli` 成功运行。
- `workspace/runs/fixture-smoke/segments/manifest.json` 指向真实 TTS 生成的 wav。
- 后续 `assemble` 可以继续使用现有 ffmpeg 阶段拼接音频。

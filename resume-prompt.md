...
- 下一步计划见 `docs/plans/03-audio-first-asr/03-real-voice-profile-provider.md`：决定这些 speaker embedding 先只保留为诊断 metadata，还是用于跨 episode speaker consistency；不要进入原主播 voice clone，也不要把 embedding 用于 TTS。

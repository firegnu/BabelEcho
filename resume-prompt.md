...
- 不再继续围绕 CosyVoice 内置两个 wav 反复微调。当前只固定使用 `cross_lingual_prompt.wav` 作为 `male_a` 的本地参考；后续如需扩展固定音色，优先微调 `CosyVoice-300M-SFT` 增加多个中文男声/女声。授权男声/中性 reference wav 只作为备用对比路线；这不是原主播 voice clone。

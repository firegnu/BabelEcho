...
- 真实播客 transcript 中常见的 `Host:` / `Guest:` / 人名冒号前缀不能长期当作正文处理。当前 plain text parser 还不会识别这类 speaker label；后续支持真实访谈和多人播客前，必须补一个轻量 speaker-label 解析或清洗层，把说话人写入 `speaker` 字段，而不是让 TTS 读出“某某：”。
...
MVP-0 不做 voice clone，但 `speaker` 字段保留，后续 MVP-1 可以按 speaker 选择音色。

真实多人播客不能长期用同一个固定中文声音读完整集。当前单声线 TTS 只适合验证端到端链路和单人/弱多人样本；后续支持两人或多人播客前，必须实现 `speaker -> voice` 映射，至少做到主持人和嘉宾使用不同固定中文音色。
...
- speaker label 只混在正文里的 transcript，除非先做 label 清洗或人工整理。
- 超长节目。
...
2. 一个硬编码英文 episode 能生成 `runs/<run-id>/output/audio.m4a`。
3. 生成的中文音频可以正常播放。
4. 每个阶段都有可检查的中间文件。
5. 失败阶段可以单独重跑，不必从头开始。
...
- speaker label 解析、speaker diarization 或人工 speaker 修正。
- 多说话人 `speaker -> voice` 映射。
- 原主播 voice clone。
- 更完整的 RSS feed 发布。

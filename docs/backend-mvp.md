...
真实多人播客不能长期用同一个固定中文声音读完整集。MVP-1 当前规则是：0/1 个 distinct speaker 且没有显式女声标签时，使用原 `CosyVoice2` cross-lingual 固定音色；单个 speaker 标签包含 `female` 或 `女` 时，自动切到 `tts.voice=sft_builtin_4role` 的 `female_a`；2 个及以上 speaker 自动切到 `tts.voice=sft_builtin_4role`，并按 speaker 首次出现顺序映射到 `female_a / male_a / female_b / male_b` 四个固定角色。
